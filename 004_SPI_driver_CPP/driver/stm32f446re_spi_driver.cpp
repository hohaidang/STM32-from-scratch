...
void spi_handler::spi_gpios_init() {
  if (spix_ == SPI1) {
    spi_sck_.gpio_init(GPIOA, GPIO_PIN_NO_5, GPIO_MODE_ALTFN, GPIO_SPEED_HIGH,
                       GPIO_OP_TYPE_PP, GPIO_NO_PUPD, IRQ_Prio_NO_15,
                       GPIO_ALT_5);

    spi_mosi_.gpio_init(GPIOA, GPIO_PIN_NO_7, GPIO_MODE_ALTFN, GPIO_SPEED_HIGH,
                        GPIO_OP_TYPE_PP, GPIO_NO_PUPD, IRQ_Prio_NO_15,
                        GPIO_ALT_5);

    spi_miso_.gpio_init(GPIOA, GPIO_PIN_NO_6, GPIO_MODE_ALTFN, GPIO_SPEED_HIGH,
                        GPIO_OP_TYPE_PP, GPIO_NO_PUPD, IRQ_Prio_NO_15,
                        GPIO_ALT_5);
    ...
    if (SPI_SSM_DI == config_.spi_ssm) {
      ...
      spi_nss_.gpio_init(GPIOA, GPIO_PIN_NO_4, GPIO_MODE_ALTFN, GPIO_SPEED_HIGH,
                         GPIO_OP_TYPE_PP, GPIO_NO_PUPD, IRQ_Prio_NO_15,
                         GPIO_ALT_5);
    }
  }
...
void spi_handler::spi_reg_config() {
  ...
  if (SPI_BUS_CONFIG_FD == config_.spi_bus_config) {
    ...
    // bidi mode should be cleared, for active SCLK, because SLCK is only
    // activate when MOSI is activate
    temp_reg &= ~(SPI_CR1_BIDIMODE);
 ...
 *
 */
void spi_handler::spi_init_nss_sw(GPIO_RegDef_t *GPIOx_addr,
                                  const u8 pin_number) {
  spi_nss_.gpio_init(GPIOx_addr, pin_number, GPIO_MODE_OUT, GPIO_SPEED_FAST,
...
void spi_handler::spi_transmit_data(const u8 *pTxBuffer, u32 Len) {
  ...
  while (spi_get_SR_reg<SPI_SR_BSY>())
    ;                   // w8 until SPI done
  spi_clear_ovr_flag(); // because in 2 lines mode, write data does not need to
                        // read
}
 ...
 *
 */
void spi_handler::spi_transmit_receive_data(const u8 *p_tx_buffer,
                                            u8 *p_rx_buffer, u32 len) {
  if (RESET == (spix_->CR1 & SPI_CR1_SPE)) {
    spi_peripheral_control<ENABLE>();
  }
  u32 init_len = len;
...
/*!
 * @brief Send data out via SPI protocol using interrupt. Interrupt will be
 * enable if hw shift buffer is empty
 *
...
/*!
 * @brief Receive data out via SPI protocol (sending padding data) using
 * interrupt. Interrupt will be enable if hw shift buffer is NOT empty
 *
...
/*!
 * @brief Send and Receive data out via SPI protocol (sending padding data)
 * using interrupt.
 *
 ...
 *
 */
void spi_handler::spi_transmit_receive_data_it(u8 *p_tx_buf, u8 *p_rx_buf,
                                               const u32 len) {
  if (RESET == (spix_->CR1 & SPI_CR1_SPE)) {
    spi_peripheral_control<ENABLE>();
  }
...
/*!
 * @brief IRQ handling. This function will be called in interrupt service
 * routine
 *
 * @param: None
 *
...
/*!
 * @brief This function should be call in interrupt service routine for handling
 * txe.
 *
 * @param: None
 *
