...
typedef enum {
  ...
  TAMP_STAMP_IRQn = 2, /*!< Tamper and TimeStamp interrupts EXTI line */
  ...
  TIM1_BRK_TIM9_IRQn = 24, /*!< TIM1 interrupt and TIM9 interrupt */
  TIM1_UP_TIM10_IRQn = 25, /*!< TIM1 Update Interrupt and TIM10 interrupt */
  TIM1_TRG_COM_TIM11_IRQn = 26, /*!< TIM1 communication and trigger irq*/
  ...
  TIM8_TRG_COM_TIM14_IRQn = 45, /*!< TIM8  and TIM14  irq*/
  ...
  TIM6_DAC_IRQn = 54,       /*!< TIM6 and DAC1&2 underrun error  interrupts */
...
typedef struct {
  __vo u32 MODER;  /*!< GPIO port mode register,                       Address
                      offset: 0x00      */
  __vo u32 OTYPER; /*!< ,                                          Address
                      offset: 0x04      */
  __vo u32 OSPEEDR;
  __vo u32 PUPDR;
  __vo u32 IDR;
  ...
  __vo u32 BSRR;
  __vo u32 LCKR;
  __vo u32 AFR[2]; /*!< AFR[0] : GPIO alternate function low register, AF[1] :
                      GPIO alternate function high register          Address
                      offset: 0x20-0x24 */
} GPIO_RegDef_t;
...
typedef struct {
  ...
  __vo u32 AHB1RSTR;   /*!< ,Addressoffset: 0x10 */
...
#define USART1_PCLK_EN() (RCC->APB2ENR |= static_cast<u32>(1UL << 4UL))
#define USART2_PCLK_EN() (RCC->APB1ENR |= static_cast<u32>(1UL << 17UL))
#define USART3_PCLK_EN() (RCC->APB1ENR |= static_cast<u32>(1UL << 18UL))
#define UART4_PCLK_EN() (RCC->APB1ENR |= static_cast<u32>(1UL << 19UL))
#define UART5_PCLK_EN() (RCC->APB1ENR |= static_cast<u32>(1UL << 20UL))
#define USART6_PCLK_EN() (RCC->APB2ENR |= static_cast<u32>(1UL << 5UL))
 ...
 ******************************************************************************************/
/*
 * Bit position definitions UART_SR
 */
#define UART_SR_PE_Pos (0UL)
#define UART_SR_PE_Msk (static_cast<u32>(1UL << UART_SR_PE_Pos))
#define UART_SR_PE (UART_SR_PE_Msk)

#define UART_SR_FE_Pos (1UL)
#define UART_SR_FE_Msk (static_cast<u32>(1UL << UART_SR_FE_Pos))
#define UART_SR_FE (UART_SR_FE_Msk)

#define UART_SR_NF_Pos (2UL)
#define UART_SR_NF_Msk (static_cast<u32>(1UL << UART_SR_NF_Pos))
#define UART_SR_NF (UART_SR_NF_Msk)

#define UART_SR_ORE_Pos (3UL)
#define UART_SR_ORE_Msk (static_cast<u32>(1UL << UART_SR_ORE_Pos))
#define UART_SR_ORE (UART_SR_ORE_Msk)

#define UART_SR_IDLE_Pos (4UL)
#define UART_SR_IDLE_Msk (static_cast<u32>(1UL << UART_SR_IDLE_Pos))
#define UART_SR_IDLE (UART_SR_IDLE_Msk)

#define UART_SR_RXNE_Pos (5UL)
#define UART_SR_RXNE_Msk (static_cast<u32>(1UL << UART_SR_RXNE_Pos))
#define UART_SR_RXNE (UART_SR_RXNE_Msk)

#define UART_SR_TC_Pos (6UL)
#define UART_SR_TC_Msk (static_cast<u32>(1UL << UART_SR_TC_Pos))
#define UART_SR_TC (UART_SR_TC_Msk)

#define UART_SR_TXE_Pos (7UL)
#define UART_SR_TXE_Msk (static_cast<u32>(1UL << UART_SR_TXE_Pos))
#define UART_SR_TXE (UART_SR_TXE_Msk)

#define UART_SR_LBD_Pos (8UL)
#define UART_SR_LBD_Msk (static_cast<u32>(1UL << UART_SR_LBD_Pos))
#define UART_SR_LBD (UART_SR_LBD_Msk)

#define UART_SR_CTS_Pos (9UL)
#define UART_SR_CTS_Msk (static_cast<u32>(1UL << UART_SR_CTS_Pos))
#define UART_SR_CTS (UART_SR_CTS_Msk)

/*
 * Bit position definitions UART_CR1
 */
#define UART_CR1_SBK_Pos (0UL)
#define UART_CR1_RWU_Pos (1UL)

#define UART_CR1_RE_Pos (2UL)
#define UART_CR1_RE_Msk (static_cast<u32>(1uL << UART_CR1_RE_Pos))
#define UART_CR1_RE (UART_CR1_RE_Msk)

#define UART_CR1_TE_Pos (3UL)
#define UART_CR1_TE_Msk (static_cast<u32>(1uL << UART_CR1_TE_Pos))
#define UART_CR1_TE (UART_CR1_TE_Msk)

#define UART_CR1_IDLEIE_Pos (4UL)
#define UART_CR1_RXNEIE_Pos (5UL)
#define UART_CR1_TCIE_Pos (6UL)
#define UART_CR1_TXEIE_Pos (7UL)
#define UART_CR1_PEIE_Pos (8UL)

#define UART_CR1_PS_Pos (9UL)
#define UART_CR1_PS_Msk (static_cast<u32>(1uL << UART_CR1_PS_Pos))
#define UART_CR1_PS (UART_CR1_PS_Msk)

#define UART_CR1_PCE_Pos (10UL)
#define UART_CR1_PCE_Msk (static_cast<u32>(1uL << UART_CR1_PCE_Pos))
#define UART_CR1_PCE (UART_CR1_PCE_Msk)

#define UART_CR1_WAKE_Pos (11UL)
#define UART_CR1_M_Pos (12UL)

#define UART_CR1_UE_Pos (13UL)
#define UART_CR1_UE_Msk (static_cast<u32>(1uL << UART_CR1_UE_Pos))
#define UART_CR1_UE (UART_CR1_UE_Msk)

#define UART_CR1_OVER8_Pos (15UL)

/*
 * Bit position definitions UART_CR2
 */
#define UART_CR2_ADD_Pos (0uL)
#define UART_CR2_LBDL_Pos (5uL)
#define UART_CR2_LBDIE_Pos (6uL)
#define UART_CR2_LBCL_Pos (8uL)
#define UART_CR2_CPHA_Pos (9uL)
#define UART_CR2_CPOL_Pos (10uL)
#define UART_CR2_CLKEN_Pos (11uL)
#define UART_CR2_STOP_Pos (12uL)
#define UART_CR2_LINEN_Pos (14uL)

/*
 * Bit position definitions UART_CR3
 */
#define UART_CR3_EIE_Pos (0uL)
#define UART_CR3_IREN_Pos (1uL)
#define UART_CR3_IRLP_Pos (2uL)
#define UART_CR3_HDSEL_Pos (3uL)
#define UART_CR3_NACK_Pos (4uL)
#define UART_CR3_SCEN_Pos (5uL)
#define UART_CR3_DMAR_Pos (6uL)
#define UART_CR3_DMAT_Pos (7uL)

#define UART_CR3_RTSE_Pos (8uL)
#define UART_CR3_RTSE_Msk (static_cast<u32>(1uL << UART_CR3_RTSE_Pos))
#define UART_CR3_RTSE (UART_CR3_RTSE_Msk)

#define UART_CR3_CTSE_Pos (9uL)
#define UART_CR3_CTSE_Msk (static_cast<u32>(1uL << UART_CR3_CTSE_Pos))
#define UART_CR3_CTSE (UART_CR3_CTSE_Msk)

#define UART_CR3_CTSIE_Pos (10uL)
#define UART_CR3_ONEBIT_Pos (11uL)
/******************************************************************************************/
...
#define USART1 (reinterpret_cast<uart_regdef_t *>(USART1_BASEADDR))
#define USART2 (reinterpret_cast<uart_regdef_t *>(USART2_BASEADDR))
#define USART3 (reinterpret_cast<uart_regdef_t *>(USART3_BASEADDR))
#define UART4 (reinterpret_cast<uart_regdef_t *>(UART4_BASEADDR))
#define UART5 (reinterpret_cast<uart_regdef_t *>(UART5_BASEADDR))
#define USART6 (reinterpret_cast<uart_regdef_t *>(USART6_BASEADDR))
#endif

/*!
 * @brief Enable Interrupt for NVIC
 *
 * @param[in] irq_number     - IRQNumber IRQ Position number
 * @param[in] en_or_di       - Enable/Disable Flag
 *
 * @return None
 *
 */
inline void config_irq(u8 irq_number, u8 en_or_di) {
  if (en_or_di == 1) {
    if (irq_number <= 31) {
...
/*!
 * @brief Configure priority, 0->15, lower number higher priority
 *
 * @param[in] irq_number: IRQ position for EXIT
 * @param[in] irq_priority: 0->15
 *
 * @return None
 *
 */
inline void config_irq_priority(u8 irq_number, u8 irq_priority) {
