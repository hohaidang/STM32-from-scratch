...
HAL_StatusTypeDef FlashNVM_Read(uint32_t const StartAddress_u32, uint8_t *DataOut_pu8, uint32_t const Size_u32);
...
#define ADDR_FLASH_PAGE_255   ((uint32_t)0x0807F800) /* Base address of Page 255, 2 Kbytes */

#define MIN_ADDR_PAGE         ADDR_FLASH_PAGE_0
#define MAX_ADDR_PAGE         ADDR_FLASH_PAGE_255
#define FLASH_ADDR_BASE       ADDR_FLASH_PAGE_0
#define FLASH_ADDR_END        ((uint32_t)0x0807FFFF)
#define MAX_NUMOFPAGE         ((uint32_t)256u)
#define MAX_SIZEOFFLASH       ((uint32_t)524288u) /*Maximum size of all pages in flash (byte): 2K *256 pages * 1024*/
#define IS_DATA_ODD(data)     (0u != (data % 2u)) /*Macro to check data is odd?*/

#define IS_FLASH_ADDRESS(ADDRESS)      ((ADDRESS >= FLASH_ADDR_BASE) && (ADDRESS <= FLASH_ADDR_END))
#endif // __FLASH_H
